...
# Helper functions for random-but-repeatable testcase generation.
...
# The actual per-problem generators start here.
...
def gen_h(rng):
    ...
    for n in sizes:
        ...
        for _ in range(q):
            if rng.random() < 0.4:
                lines.append(f"1 {rng.randint(1, n)} {rng.randint(1, 10**9)}")

            else:
                lines.append(f"2 {rng.randint(1, n)}")
